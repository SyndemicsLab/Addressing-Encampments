...
time_varying_blk_trans_cycles <<- c(52, 53, 65, 104) # each element should have its own matrix. Each of these matrices represents a time interval. Only the upper limit is inclusive.

# overdose parameters
# all types overdose and fatal to all types overdose ratios are considered to have the same time_varying intervals
time_varying_overdose_cycles <<- c(52, 53, 65, 104)    # each element should have its own column(for all types) or row (for fatal). Each of them represents a time interval, from last cycle to current cycle. Only the upper limit is inclusive
