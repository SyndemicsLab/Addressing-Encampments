...
# Last updated 12/11/2023 by Hana

# Outputs of interest: total fatal overdoses, total overdoses, cost, QALYs
...
output_file_names <- c("general_stats1.csv", 
                       ...
                       "cost_life/healthcare_utilization_cost1.csv")
...
all_cause_mortality_stats <- data.frame(median_400, sd_400, lower_400, upper_400, avg_percent_dead)
...
sd_fod <- sd_fod/507.19

fatal_od_stats <- data.frame(median_400, sd_fod, lower_400, upper_400, median_fod, lower_new_fod, upper_new_fod)
...
# for cost, we need total_costs1.csv
# no summing necessary, just pull the value

# code Hana started with
# grouped_runs[["output1"]]$"cost_life/total_costs1.csv"[1,"total"]
...
#  cost[((k - 1) %% length(cost)) + 1] <- grouped_runs[[folderName]]$"cost_life/total_costs1.csv"[2, ]
...
median_cost <- quantile(cost, probs = c(0.5))
lower_new_cost <- quantile(cost, probs = c(0.025))
upper_new_cost <- quantile(cost, probs = c(0.975))
...
# for time in MOUD, we need total population in bup/ntx/mmt from general_stats1, sum of cycles
# columns = 0 (1) to 104 (105), rows = Buprenorphine/Naltrexone/Methadone
...
#  tm[((p - 1) %% length(tm)) + 1] <- sum(rowSums(current_stats[2, grepl("total_size_cycle", names(current_stats))])) + sum(rowSums(current_stats[3, grepl("total_size_cycle", names(current_stats))])) + sum(rowSums(current_stats[4, grepl("total_size_cycle", names(current_stats))]))
  p <- p + 1
}
...
# csv: cost_life/healthcare_utilization_cost1.csv
# all of row two, summed
...
for (folder in grouped_runs) {
  folderName <- paste0("output", q)
  current_stats <- grouped_runs[[folderName]][["cost_life/healthcare_utilization_cost1.csv"]]
  hcu[((q - 1) %% length(cost)) + 1] <- rowSums(current_stats[2, ])
  q <- q +1
}
...
# calculate median/95% CI and output those to a csv

median_hcu <- quantile(hcu, probs = c(0.5))
lower_new_hcu <- quantile(hcu, probs = c(0.025))
upper_new_hcu <- quantile(hcu, probs = c(0.975))

median_400 <- median_hcu/507.19
lower_400 <- lower_new_hcu/507.19
upper_400 <- upper_new_hcu/507.19

print("hcu")
print(median_400)

hcu_stats <- data.frame(median_400, lower_400, upper_400, median_hcu, lower_new_hcu, upper_new_hcu)

write.csv(hcu_stats, "hcu_stats.csv", row.names=FALSE)
