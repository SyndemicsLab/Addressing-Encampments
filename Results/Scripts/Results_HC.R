...
# Time in Housing (for housing with meds AND housing without meds)

# for time in housing, we need total population in housing from general_stats1, sum of cycles
# columns = 0 (1) to 104 (105), row = housing
...
for (folder in grouped_runs) {
  ...
  th[((n - 1) %% length(th)) + 1] <- sum(rowSums(current_stats[8:9, paste0("total_size_cycle", year2)]))
#  th[((n - 1) %% length(th)) + 1] <- sum(rowSums(current_stats[8, grepl("total_size_cycle", names(current_stats))])) + sum(rowSums(current_stats[9, grepl("total_size_cycle", names(current_stats))]))
  n <- n + 1
}
...
#  acm[((h - 1) %% length(acm)) + 1] <- (sum(current_stats[, "total_size_cycle0"]) - sum(current_stats[, "total_size_cycle104"]))
  h <- h + 1
}
...
print("dataframe for fatal od stats")
fatal_od_stats
...
median_cost <- quantile(cost, probs = c(0.5)) + 7813988.27 # sweep one-time cost based on wk 53 pop of 28,419.67
lower_new_cost <- quantile(cost, probs = c(0.025)) + 7813988.27 # sweep one-time cost based on wk 53 pop of 28,419.67
upper_new_cost <- quantile(cost, probs = c(0.975)) + 7813988.27 # sweep one-time cost based on wk 53 pop of 28,419.67
...
# columns = 0 (1) to 104 (105), rows = Buprenorphine/Naltrexone/Methadone/housing_meds
...
for (folder in grouped_runs) {
  ...
  current_stats <- grouped_runs[[folderName]][["general_stats1.csv"]]
  tm[((p - 1) %% length(tm)) + 1] <- sum(rowSums(current_stats[c(2,3,4,8), paste0("total_size_cycle", year2)]))
